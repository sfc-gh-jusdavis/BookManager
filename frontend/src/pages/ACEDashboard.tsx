...
function WidgetCard({
  ...
  return (
    <div
      className={`bg-white rounded-xl border border-slate-200 shadow-sm col-span-${span}`}
...
const PRIORITY_RANK: Record<NextAction['priority'], number> = {
  high: 0,
  medium: 1,
  low: 2,
}
...
export function ACEDashboard() {
  ...
  const myAccounts = useMemo(
    () => MOCK_ACCOUNTS.filter((a) => isAceOnAccount(a, aceId)),
    [aceId],
  )

  const accountById = useMemo(() => {
    const m = new Map<string, Account>()
    for (const a of MOCK_ACCOUNTS) m.set(a.account_id, a)
    return m
  }, [])

  const myAccountIds = useMemo(
    () => new Set(myAccounts.map((a) => a.account_id)),
    [myAccounts],
  )

  const myUseCases = useMemo(
    () => MOCK_USE_CASES.filter((uc) => myAccountIds.has(uc.account_id)),
    [myAccountIds],
  )

  const aceAccountIds = useMemo(
    () => new Set(myAccounts.map((a) => a.account_id)),
    [myAccounts],
  )
  ...
  const atRiskAlerts = useMemo((): AtRiskAlertRow[] => {
    ...
    for (const uc of myUseCases) {
      ...
      if (!account) continue
      const blocked = uc.status === 'Blocked'
      const accountAtRisk = account.status === 'At Risk'
      if (!blocked && !accountAtRisk) continue
      rows.push({
        useCase: uc,
        account,
        dotClass: blocked ? 'bg-red-500' : 'bg-amber-500',
      })
    }
    return rows
  ...
  const pipelineData = useMemo((): PipelineRow[] => {
    ...
    for (const uc of myUseCases) {
      const n = counts.get(uc.stage) ?? 0
      counts.set(uc.stage, n + 1)
    }
  ...
  const creditChartData = useMemo((): CreditChartRow[] => {
    ...
    const perAccount = myAccounts.map((acc) =>
      getCreditSeriesForAccount(acc.account_id).slice(-30),
    )
    const first = perAccount[0]
    if (!first) return []
    const len = first.length
    const out: CreditChartRow[] = []
    for (let i = 0; i < len; i++) {
      const entry = first[i]
      if (!entry) continue
      const row: CreditChartRow = { date: entry.date }
      for (let j = 0; j < myAccounts.length; j++) {
        const acc = myAccounts[j]
        const series = perAccount[j]
        const seriesEntry = series?.[i]
        if (acc && seriesEntry) {
          row[acc.account_name] = seriesEntry.credits_used
        }
      }
      out.push(row)
    }
  ...
  const accountNamesForLines = useMemo(
    () => myAccounts.map((a) => a.account_name),
    [myAccounts],
  )
  ...
  const upcomingGoLives = useMemo(() => {
    ...
    return [...myUseCases]
      ...
      .sort(
        (a, b) =>
          parseLocalDate(a.target_go_live_date!).getTime() -
          parseLocalDate(b.target_go_live_date!).getTime(),
      )
      .slice(0, 5)
  }, [myUseCases])
  ...
  const nextBestActions = useMemo((): NextAction[] => {
    ...
    for (const uc of myUseCases) {
      if (uc.status === 'Blocked') {
        actions.push({
          id: `blocked-${uc.use_case_id}`,
          accountId: uc.account_id,
          Icon: AlertTriangle,
          iconClassName: 'text-red-500',
          text: `Resolve blocker for ${uc.use_case_name} at ${uc.account_name}`,
          priority: 'high',
        })
      }
    }
    ...
    for (const acc of myAccounts) {
      if (acc.status === 'At Risk') {
        actions.push({
          id: `risk-${acc.account_id}`,
          accountId: acc.account_id,
          Icon: AlertTriangle,
          iconClassName: 'text-amber-500',
          text: `Review health of ${acc.account_name}`,
          priority: 'medium',
        })
      }
    }
    ...
    for (const uc of myUseCases) {
      ...
      if (target.getTime() > in30.getTime()) continue
      actions.push({
        id: `golive-${uc.use_case_id}`,
        accountId: uc.account_id,
        Icon: CalendarClock,
        iconClassName: 'text-sky-600',
        text: `Prepare go-live checklist for ${uc.use_case_name}`,
        priority: 'medium',
      })
    }

    const cutoff = today.getTime() - 14 * MS_PER_DAY
    for (const call of MOCK_GONG_CALLS) {
      if (!aceAccountIds.has(call.account_id)) continue
      const callTime = new Date(call.call_date).getTime()
      if (callTime < cutoff || callTime > Date.now()) continue
      const account = accountById.get(call.account_id)
      const accountName = account?.account_name ?? call.account_id
      for (const item of call.action_items) {
        if (item.trimStart().startsWith('[DONE]')) continue
        actions.push({
          id: `gong-${call.call_id}-${item.slice(0, 24)}`,
          accountId: call.account_id,
          Icon: Zap,
          iconClassName: 'text-violet-500',
          text: `Follow up: ${item} (${accountName})`,
          priority: 'low',
        })
      }
    }
  ...
  }, [myUseCases, myAccounts, aceAccountIds, accountById])
  ...
  return (
    <>
      <Header
        title={`Welcome back, ${currentUser.display_name.split(' ')[0]}`}
        subtitle="Here's how your accounts are looking today"
      />
      <div className="p-6">
        <div className="grid grid-cols-12 gap-4">
          <WidgetCard title="My Accounts" icon={<Building2 size={16} />} span={8}>
            {myAccounts.length === 0 ? (
              ...
              <div className="divide-y divide-slate-100">
                {myAccounts.map((acc) => {
                  const series = getCreditSeriesForAccount(acc.account_id)
                  const last = series[series.length - 1]
                  const used = last?.credits_used ?? 0
                  const creditsLabel = `${formatCreditsShort(used)} / ${formatCreditsShort(acc.total_credits_allocated)}`
                  return (
                    <Link
                      key={acc.account_id}
                      to={`/accounts/${acc.account_id}`}
                      className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-x-3 gap-y-0.5 items-center py-2 text-xs hover:bg-slate-50 -mx-2 px-2 rounded-md transition-colors"
                    >
          ...
          <WidgetCard title="At-Risk Alerts" icon={<AlertTriangle size={16} />} span={4}>
            {atRiskAlerts.length === 0 ? (
              <p className="text-xs text-emerald-600 font-medium text-center py-4">
                No alerts — all accounts on track
              </p>
            ) : (
                ...
                >
                  ...
                  <Bar dataKey="count" radius={[0, 4, 4, 0]}>
                    {pipelineData.map((entry, i) => (
                      <Cell
                        key={entry.stage}
                        fill={PIPELINE_BAR_FILLS[i % PIPELINE_BAR_FILLS.length]}
                      />
                    ))}
                  </Bar>
                </BarChart>
          ...
          <WidgetCard title="Credit Trends" icon={<TrendingUp size={16} />} span={8}>
            {creditChartData.length === 0 ? (
              <p className="text-xs text-slate-400 text-center py-8">
                No credit data for your accounts.
              </p>
            ) : (
              <ResponsiveContainer width="100%" height={200}>
                ...
                >
                  ...
                  />
                  <Tooltip
                    formatter={creditTooltipFormatter}
              ...
              <ul className="space-y-2.5">
                {upcomingGoLives.map((uc) => {
                  ...
                  return (
                    <li
                      key={uc.use_case_id}
                      className="flex flex-wrap items-baseline justify-between gap-x-2 gap-y-1 border-b border-slate-100 pb-2 last:border-0 last:pb-0"
                    >
                      ...
                      <div className="flex flex-col items-end gap-0.5 shrink-0">
                        <span className="text-xs text-slate-600">
                          {formatGoLiveLabel(uc.target_go_live_date!)}
                        </span>
                        <span className={`text-xs font-medium tabular-nums ${dayColor}`}>
                          {days} days
                        </span>
          ...
          <WidgetCard title="Next Best Actions" icon={<Zap size={16} />} span={6}>
            {nextBestActions.length === 0 ? (
              ...
              <ul className="space-y-2">
                {nextBestActions.map(
                  ({ id, accountId: actAccountId, Icon: ActionIcon, iconClassName, text, priority }) => (
                    <li
                      key={id}
                      className="flex gap-2 items-start border-b border-slate-100 pb-2 last:border-0 last:pb-0"
                    >
                      <ActionIcon
                        size={16}
                      ...
                      <div className="min-w-0 flex-1">
                        <Link
                          to={`/accounts/${actAccountId}?tab=assistant&prompt=${encodeURIComponent(text)}`}
                          className="text-sm text-slate-700 leading-snug hover:text-snow-600 hover:underline"
                        >
                          {text}
                        </Link>
                        <span
                          className={`inline-block mt-1 rounded px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wide ${priorityChipClass(priority)}`}
                        >
                          {priority}
                        </span>
                      </div>
