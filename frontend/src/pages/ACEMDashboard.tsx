...
const JANE_DISPLAY = 'Jane Smith'
const CARLOS_DISPLAY = 'Carlos Rodriguez'
...
export function ACEMDashboard() {
  const kpis = useMemo(() => {
    ...
    const onTrackCount = MOCK_ACCOUNTS.filter(
      (a) => a.status === 'Active' || a.status === 'Go Live'
    ).length
    const onTrackPct =
      totalAccounts > 0 ? Math.round((onTrackCount / totalAccounts) * 100) : 0
    const atRiskAccounts = MOCK_ACCOUNTS.filter((a) => a.status === 'At Risk').length
    const goLivesMarch2026 = MOCK_USE_CASES.filter((uc) => {
  ...
  const teamMemberCards = useMemo(() => {
    ...
    return aceIds.map((aceId) => {
      ...
      const atRiskCount = blockedCount + atRiskAccountCount
      const deployedCount = useCases.filter((uc) => uc.stage === 'Deployed').length
      const inProgressCount = useCases.filter(
        (uc) => uc.stage === 'Impl In Progress' || uc.stage === 'Impl Pending',
      ).length
      const goLiveThisQ = useCases.filter((uc) => {
        const d = uc.target_go_live_date
        return d !== null && d >= '2026-04-01' && d <= '2026-06-30'
      }).length
  ...
  const creditTrendData = useMemo(() => {
    ...
    return dateStrings.map((dateStr) => {
      ...
      for (const acc of MOCK_ACCOUNTS) {
        ...
        if (!row) continue
        if (acc.ace_assigned === 'ace-jane') janeTotal += row.credits_used
        else if (acc.ace_assigned === 'ace-carlos') carlosTotal += row.credits_used
      }
      return {
        date: dateStr,
        [JANE_DISPLAY]: janeTotal,
        [CARLOS_DISPLAY]: carlosTotal,
      }
    })
  }, [])

  const pendingAdjustments = useMemo(() => {
    const pending = MOCK_FORECASTS.filter((f) => f.pending_approval)
    return pending.map((f) => {
      const uc = MOCK_USE_CASES.find((u) => u.use_case_id === f.use_case_id)
      const submitterName = ACE_DISPLAY_NAMES[f.override_by ?? ''] ?? f.override_by ?? 'Unknown'
      return {
        ...f,
        useCaseName: uc?.use_case_name ?? f.use_case_id,
        accountName: uc?.account_name ?? f.account_id,
        submitterName,
        fromCategory: f.auto_category,
        toCategory: effectiveCategory(f),
      }
    })
  }, [])
  ...
  const managerAlerts = useMemo((): ManagerAlert[] => {
    ...
    for (const uc of MOCK_USE_CASES) {
      ...
      cur.count += 1
      blockedMap.set(uc.account_id, cur)
    }
    ...
    const overdueAlerts: ManagerAlert[] = MOCK_USE_CASES.filter(
      (uc) => uc.target_go_live_date !== null && uc.target_go_live_date < TODAY_ISO
    ).map((uc) => ({
      id: `overdue-${uc.use_case_id}`,
      kind: 'overdue' as const,
      message: `${uc.use_case_name} at ${uc.account_name} is past target go-live`,
      accountName: uc.account_name,
      accountId: uc.account_id,
    }))

    const atRiskAlerts: ManagerAlert[] = MOCK_ACCOUNTS.filter((a) => a.status === 'At Risk').map(
      (a) => ({
        id: `atrisk-${a.account_id}`,
        kind: 'at-risk' as const,
        message: `${a.account_name} marked as At Risk`,
  ...
  return (
    <>
      <Header
        title="Team Overview"
        subtitle="Account health across your team"
      />
      <div className="p-6">
        <div className="grid grid-cols-12 gap-4">
          <KPICard
            ...
            color="bg-snow-50 text-snow-600"
          />
          <KPICard
            label="On Track"
          ...
          />
          <KPICard
            label="Go-Lives This Month"
            value={kpis.goLivesMarch2026}
              ...
              {teamMemberCards.map((member) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-snow-600 opacity-0 group-hover:opacity-100 transition-opacity">
                      View Profile →
                    </span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-sm">
                    <div>
                      ...
                      <p className="font-semibold text-slate-800">{member.accountCount}</p>
                    </div>
                    <div>
                      <span className="text-xs text-slate-500">Use Cases</span>
                      <p className="font-semibold text-slate-800">{member.useCaseCount}</p>
                    </div>
                    <div>
                      <span className="text-xs text-slate-500">Deployed</span>
                      <p className="font-semibold text-emerald-600">{member.deployedCount}</p>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2 text-xs">
                    ...
                    <span className="text-slate-500">
                      {member.goLiveThisQ} go-live this Q
                    </span>
                  ...
                  <div className="flex flex-wrap gap-1.5 items-center">
                    {member.accounts.map((acc) => (
                      <span
                        key={acc.account_id}
                        title={`${acc.account_name}: ${acc.status}`}
                        className={`h-2.5 w-2.5 rounded-full shrink-0 ${accountStatusDotClass(acc.status)}`}
                      />
                    ))}
                  </div>
                </Link>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={creditTrendData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                ...
                <XAxis
                  ...
                  tick={{ fontSize: 10 }}
                  stroke="#64748b"
                  tickFormatter={formatChartDateMd}
                />
                <YAxis
                  tick={{ fontSize: 10 }}
                  stroke="#64748b"
                  tickFormatter={formatAxisDateK}
                />
                <Tooltip
                  contentStyle={{
                    fontSize: 12,
                  ...
                  formatter={(value: number | string, name: string) => [
                    formatAxisDateK(Number(value)),
                    name,
                  ]}
                />
                <Area
                  type="monotone"
                  dataKey={JANE_DISPLAY}
                  stackId="credits"
                  stroke="#29B5E8"
                  fill="#29B5E8"
                />
                  ...
                  <li key={alert.id} className="flex gap-2 items-start">
                    <span
                      className={`mt-1.5 h-2 w-2 rounded-full shrink-0 ${
                        alert.kind === 'blocked' ? 'bg-red-500' : 'bg-amber-500'
                      }`}
                    />
                    <div className="min-w-0 flex-1">
                      <Link
                        to={`/accounts/${alert.accountId}?tab=assistant&prompt=${encodeURIComponent(alert.message)}`}
                        className="text-sm text-slate-800 hover:text-snow-600 hover:underline"
                      >
                        {alert.message}
                      </Link>
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-1 text-xs text-slate-500">
                            ...
                            {' · '}
                            <span
                              className="inline-flex rounded-full px-1.5 py-0.5 text-[10px] font-medium bg-slate-100 text-slate-600"
                            >
                              {adj.fromCategory}
                            </span>
                            {' → '}
                            <span
                              className={`inline-flex rounded-full px-1.5 py-0.5 text-[10px] font-medium ${
                                adj.toCategory === 'Commit'
                                  ? 'bg-emerald-50 text-emerald-700'
                                  : adj.toCategory === 'Most Likely'
