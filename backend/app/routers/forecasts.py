...
from app.models.prediction import CreditForecast, SimilarDeployment, UseCaseCompletionPrediction, UseCaseForecast
...
@router.get("/use-cases-forecast", response_model=list[UseCaseForecast])
async def list_use_case_forecasts(
    user: CurrentUser = Depends(get_current_user),
    data: SnowflakeDataService = Depends(get_data_service),
) -> list[UseCaseForecast]:
    return data.list_use_case_forecasts(_ace_filter(user), _acem_filter(user))
...
@router.get("/consumption-projection")
async def get_consumption_projection(
    user: CurrentUser = Depends(get_current_user),
    data: SnowflakeDataService = Depends(get_data_service),
) -> dict:
    return data.get_consumption_projection(_ace_filter(user), _acem_filter(user))
