CREATE OR REPLACE PROCEDURE TEMP.JUSDAVIS.SP_COMPUTE_AI_ASSESSMENTS()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
BEGIN
  -- MERGE 1: per-use-case AI assessments
  -- Defensive QUALIFY ensures one row per USE_CASE_ID even if upstream fanouts.
  ...
  USING (
    ...
    QUALIFY ROW_NUMBER() OVER (PARTITION BY USE_CASE_ID ORDER BY COMPUTED_AT DESC) = 1
  ) src
  ...
  -- MERGE 2: per-account AI assessments
  -- Defensive QUALIFY ensures exactly one source row per ACCOUNT_ID. Historically
  -- this MERGE failed daily with error 100090 (Duplicate row detected during DML
  -- action) when BKMNG_ONT_ACCOUNTS had a duplicate row for 001VI00000B2bBdYAJ
  -- caused by upstream A360_OVERAGE_UNDERAGE_PREDICTION_VIEW fanout. The root
  -- cause is fixed in SP_REFRESH_BKMNG_A360_CONTRACT; this QUALIFY is defense
  -- in depth so a future upstream regression cannot break the AI pipeline.
  ...
  USING (
    ...
    QUALIFY ROW_NUMBER() OVER (PARTITION BY ACCOUNT_ID ORDER BY COMPUTED_AT DESC) = 1
  ) src
