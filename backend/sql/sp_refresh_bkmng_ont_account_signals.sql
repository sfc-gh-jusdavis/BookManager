...
BEGIN
    ...
    -- cadence_slipping (weekly cadence missed; 8-13 days since last meeting; active use case; no upcoming meeting on calendar)
    INSERT INTO TEMP.JUSDAVIS.BKMNG_ONT_ACCOUNT_SIGNALS
        (SIGNAL_ID, ACCOUNT_ID, ACCOUNT_NAME, SIGNAL_TYPE, PRIORITY, SIGNAL_TEXT, CONTEXT, ENTITY_TYPE, ENTITY_ID, CREATED_AT)
    SELECT
        'cadence_slipping-' || a.ACCOUNT_ID,
        a.ACCOUNT_ID, a.ACCOUNT_NAME, 'cadence_slipping', 'medium',
        'Weekly cadence slipping at ' || a.ACCOUNT_NAME || ': ' ||
            COALESCE(DATEDIFF('day', a.LAST_MEETING_DATE, CURRENT_TIMESTAMP())::VARCHAR, '14+') || ' days since last meeting',
        'Last meeting: ' || COALESCE(a.LAST_MEETING_DATE::VARCHAR, 'never') ||
            '. Meetings (30d): ' || COALESCE(a.MEETINGS_LAST_30D::VARCHAR, '0') ||
            '. Upcoming (5d): ' || COALESCE(a.UPCOMING_MEETINGS_5D::VARCHAR, '0') ||
            '. Active use cases: ' || COALESCE(a.ACTIVE_USE_CASE_COUNT::VARCHAR, '0') ||
            '. Target cadence: 1+ meeting/week (Q1 data: weekly cadence doubles go-live rate).',
        'account', a.ACCOUNT_ID, CURRENT_TIMESTAMP()
    FROM TEMP.JUSDAVIS.BKMNG_ONT_ACCOUNTS a
    WHERE a.ACTIVE_USE_CASE_COUNT > 0
      AND COALESCE(a.UPCOMING_MEETINGS_5D, 0) = 0
      AND a.ENGAGEMENT_STATUS NOT IN ('Churned', 'Renewal')
      AND a.STATUS NOT IN ('Churned')
      AND (
          DATEDIFF('day', a.LAST_MEETING_DATE, CURRENT_TIMESTAMP()) BETWEEN 8 AND 13
          OR (a.LAST_MEETING_DATE IS NULL
              AND a.LAST_EXTERNAL_INTERACTION_DATE >= DATEADD('day', -30, CURRENT_DATE()))
      );
    ...
    SET SOURCE = 'core',
        CATEGORY = CASE SIGNAL_TYPE
            ...
            WHEN 'cadence_slipping'     THEN 'engagement'
        ...
        ALERT_ELIGIBLE = CASE
            ...
            WHEN SIGNAL_TYPE IN ('open_tmr', 'new_feature_adoption', 'no_upcoming_meeting', 'email_silence', 'cadence_slipping', 'use_case_no_go_live', 'use_case_no_impl_start', 'use_case_stale_notes', 'champion_silent') THEN TRUE
            ELSE FALSE
        END
    WHERE SOURCE = 'core' AND CATEGORY IS NULL;
