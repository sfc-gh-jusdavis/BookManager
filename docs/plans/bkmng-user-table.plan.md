...
4. Backend: `get_current_user()` checks `X-Mock-User` first → looks up row in `BKMNG_USERS` → returns profile with correct email (`redacted@example.com`) and role (`acem`)
...
INSERT INTO TEMP.JUSDAVIS.BKMNG_USERS VALUES
    ('ufitolo',  'Ufi Olakpe',     'redacted@example.com',     'acem', false),
    ('gilee',    'Gilbert Lee',    'redacted@example.com',    'acem', false),
    ('dbaccus',  'Daunte Baccus',  'redacted@example.com',  'acem', false),
    ('jusdavis', 'Justin Davis',   'redacted@example.com',        'ace',  false);
```
