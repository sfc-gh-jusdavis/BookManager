...
**Root cause**: `a.ace_assigned === currentUser?.user_id` uses the mock user ID (e.g. `jusdavis-ace`) but `BKMNG_ACCOUNTS.ACE_ASSIGNED` stores the ACE's email (e.g. `j.davis@snowflake.com`). `ACEDashboard.tsx` correctly uses `currentUser?.email`.
